...
func main() {
	...
	// 2. Retry Transport (rehttp) - This logic is now inside customTransport
	...
	//    Wraps the baseHttpTransport and includes retry logic internally.
	customTransportWrapper := transport.NewCustomTransport(
		apiKey,            // apiKey
		backendID,         // backendID
		traceparent,       // clientTraceparent
		enableGzip,        // clientGzipEnabled
		baseHttpTransport, // baseHttpTransport (pre-retry)
		defaultRetryCount, // retryCount
		minRetryWait,      // minWait
		maxRetryWait,      // maxWait
		// Pass nil or an empty slice for default retry statuses, or specify: []int{http.StatusServiceUnavailable, http.StatusTooManyRequests}
		[]int{http.StatusServiceUnavailable, http.StatusTooManyRequests, http.StatusGatewayTimeout, http.StatusBadGateway}, // retryStatuses
	)
